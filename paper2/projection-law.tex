...
explicit rate $3(d-1)/\rho$; the participation ratio is provably blind to
sign structure that pairwise alignment detects; and the chain is extended to
closed affine reachable sets (bias--within-family decomposition), $C^1$ smooth
non-convex immersions (local normal cone), and finite-sample concentration of
the empirical second-moment matrix used by the gauge. We then test the law's sharpest
...
layer --- median participation ratio $1.09$ (classical), $1.3$ (FCC foundation MLIPs), and $1.10$ (DFT) out of 3
...
Theorem~\ref{thm:affine} turns the bias-plus-noise spectrum of
...
Theorem~\ref{thm:smooth} licenses the neural-network reachable sets used
below: the conclusion is pointwise (one normal space per local minimizer), not
the global consensus theorem, and empirical clustering tests that normal-space
membership.
...
The complete kit, pinned datasets, and pre-registrations are publicly served at \url{https://storage.googleapis.com/shed-489901-replication/error-geometry/v1-10c18ace/index.html}. The Lean~4 artifact (225 theorem and lemma declarations in the current
