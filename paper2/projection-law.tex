...
the next constraint upstream in the epistemic stack. This is the part of the
law for which we find no antecedent in the climate, forecasting, machine
learning, or materials literatures, and it is the part we test hardest.
...
The algebra is a one-line corollary of spiked-covariance theory and the
participation ratio is a standard effective-dimension metric; the value here
is the consilience it enables (\S\ref{sec:classical}) and the machine-checked
monotonicity that licenses inverting measured PR into a systematic fraction
$\alpha = \rho/(\rho+1)$.
...
with the replication kit (\S\ref{sec:repro}).
...
That ensembles share errors is established: error consistency across networks
\citep{geirhos2020}, model similarity beyond accuracy \citep{mania2019},
prediction-space clustering \citep{fort2019}, accuracy- and
agreement-on-the-line \citep{baek2022}. That consensus is not verification is
the qualitative lesson of \citet{pirtle2010} and \citet{parker2011},
operationalized as error correlation by \citet{bishop2013}; low-dimensional
structure of multi-model bias appears in EOF analyses
\citep{sanderson2015,brands2022}. The participation ratio and the
spiked-covariance algebra behind Theorem~\ref{thm:gauge} are standard. In
materials simulation, shared softening of universal MLIPs is documented
\citep{deng2025softening,yu2024}, reproducibility of DFT codes is measured by
scalar metrics \citep{lejaeghere2016,bosoni2024}, and parameter-space
sloppiness of single potentials is well developed
\citep{transtrum2011,kurniawan2022,quinn2023}. Relative to all of the above,
the contributions here are: the error-vector geometry of model ensembles in
observable space as the measured object; the factorial
constraint-vs-implementation designs and their pre-registered outcomes; the
conservation--rotation law; and the machine-checked theory chain with a
two-tier replication kit.
