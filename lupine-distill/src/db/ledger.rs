...
//!
//! In `lupine-distill` this module is reached only through `import_ledger`
//! (used by the `db migrate-ledger` CLI command) — most of the helper API is
//! preserved for future migrations and inter-agent coordination, so per-symbol
