...
const DEMO_MANIFEST = resolve(CWD, '../nist_ipr/demos/manifest.json');
...
function main() {
  ...
  if (existsSync(DEMO_MANIFEST)) {
    demos = JSON.parse(readFileSync(DEMO_MANIFEST, 'utf-8'));
    console.log(`  Joined ${Object.keys(demos).length} demo entries`);
  } else {
    console.log('  No demo manifest found (run demo generation first)');
  }
