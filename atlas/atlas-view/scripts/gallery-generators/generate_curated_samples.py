...
OUTPUT_DIR = Path(__file__).resolve().parents[2] / "apps" / "web" / "public" / "archive"
...
d1, b1 = gen_hea()
write_lammpstrj(OUTPUT_DIR / "curtin_hea_dislocation.lammpstrj", len(d1), b1, d1)
print(f"Generated HEA ({len(d1)} atoms)")

d2, b2 = gen_llzo()
write_lammpstrj(OUTPUT_DIR / "llzo_diffusion.lammpstrj", len(d2), b2, d2)
print(f"Generated LLZO ({len(d2)} atoms)")

with open(OUTPUT_DIR / "zif8_gate_opening.lammpstrj", 'w') as f:
    frames = 120
...
d4, b4 = gen_cascade()
write_lammpstrj(OUTPUT_DIR / "w_cascade_150keV.lammpstrj", len(d4), b4, d4, "id type x y z damage")
print(f"Generated W Cascade ({len(d4)} atoms)")

d5, b5 = gen_gst()
write_lammpstrj(OUTPUT_DIR / "gst_crystallization.lammpstrj", len(d5), b5, d5)
