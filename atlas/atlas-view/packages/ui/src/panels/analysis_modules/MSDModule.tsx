...
export function MSDModule() {
  const { colorMode, colorProperty, setColorMode, setColorProperty, setColormap } = useStore();
  const isActive = colorMode === 'property' && colorProperty === 'msd_displacement';
  ...
  return (
    <QuantumSection label="Mean Squared Displacement" defaultOpen={true}>
      ...
      <button
        onClick={() => {
          if (isActive) {
            ...
            setColorProperty('msd_displacement');
      ...
      >
        <IconChart /> {isActive ? 'Hide MSD' : 'Compute MSD'}
      </button>
    </QuantumSection>
  );
}
