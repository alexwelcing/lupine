...
import { deriveFacets } from './providers/omol';
...
const FIXTURE = [
  ...
  { id: 'nval-2', formula: 'C2H6O', elements: ['C', 'H', 'O'], natoms: 9, gap: null, energy: -154, src: 'DS_x' },
];
...
describe('OMol25 — deriveFacets', () => {
  it('counts each element across the structures that contain it', () => {
    ...
    expect(count('H')).toBe(3); // in all three
    expect(count('C')).toBe(2); // CH4 + C2H6O
    expect(count('O')).toBe(2); // H2O + C2H6O
  });
  ...
  it('reports the atom-count range and median', () => {
    ...
    expect(f.natoms.max).toBe(9);
    expect(f.natoms.median).toBe(5);
  });
  ...
  it('does NOT derive a gap or src facet (both are unusable in this slice)', () => {
    ...
    // only the three real facets exist
    expect(Object.keys(f).sort()).toEqual(['elementCounts', 'natoms', 'total']);
  });
