...
// in the vertex shader, with the fragment shader computing:
//   - Sphere surface intersection (ray-sphere test)
//   - Correct depth buffer value (for proper occlusion)
//   - Surface normal (for lighting)
//   - Material color (from per-atom property)
//
// This is orders of magnitude faster than tessellated mesh spheres
// because it requires only 4 vertices per atom vs. hundreds for meshes.
// ═══════════════════════════════════════════════════════════════════
...
struct Camera {
    ...
    near:       f32,
    far:        f32,
    // Viewport dimensions for depth linearization
    width:      f32,
    height:     f32,
    _pad:       f32,
};
...
// Atom radii (one per atom, in Angstroms)
@group(1) @binding(1) var<storage, read> radii: array<f32>;

// Atom colors (RGBA packed as u32, or separate float4 array)
...
fn vs_main(
    ...
    // Quad corner offsets (billboard facing camera)
    // Vertex order: 0=BL, 1=BR, 2=TL, 3=TR (triangle strip: 0,1,2, 2,1,3)
    let quadCorners = array<vec2<f32>, 4>(
        vec2<f32>(-1.0, -1.0),
        vec2<f32>( 1.0, -1.0),
        vec2<f32>(-1.0,  1.0),
        vec2<f32>( 1.0,  1.0),
    ...
    // Use slightly larger than radius to account for perspective edge
...
fn fs_main(in: VertexOutput) -> FragOutput {
    ...
    // Ray origin: camera position in view space (0,0,0)
    // Ray direction: towards the fragment position in view space
    ...
    let oc = -in.viewCenter; // camera is at origin in view space
