...
\textbf{Results:} Prediction errors universally occupy hyper-ribbon manifolds with effective dimensionality $1.05$--$1.86$ out of 3, consistent with the sloppy model universality class. An empirical extension of the analysis to three foundation machine-learning interatomic potentials (MACE-MP-0, CHGNet, Orb-v3) confirms that 14 of 15 elements retain participation ratio $<2.0$ when foundation MLIPs are added to the per-element ensembles, with Fe persisting as the lone outlier across all interventions---supporting hyper-ribbon universality across both classical and foundation-MLIP modeling paradigms. A striking crystal-structure dichotomy emerges: all 7 BCC metals show reference--prediction correlations $r>0.70$ (mean $r=0.89$), while 7 of 8 FCC metals show $r<0.40$ (mean $r=0.16$). Meta-analysis across 15 elements ($N=1{,}677$) reveals extreme heterogeneity ($I^2=98.6\%$, $\tau=0.80$), with a 95\% prediction interval spanning $[-0.64, +0.99]$. A direct test of cross-style PC1 alignment finds that the apparent per-element dichotomy is dominated by a sample-size confounder (Spearman $\rho(n_{\text{pairs}}, \overline{\cos}) = -0.50$, $p = 0.06$) rather than $d$-band fullness, with a residual d-band signal emerging only after fitting depth is controlled ($\rho = +0.52$, $p = 0.087$, $n=12$); this classical-family-specific dichotomy does not transfer across modeling paradigms.
...
The two findings together resolve the apparent contradiction: removing the same three single-pair outliers from the d-band test reverses the sign of $\rho$ to $\rho(d_{\text{count}}, \overline{\cos}) = +0.523$ ($p = 0.087$, $n=12$). On a controlled subset, a residual d-band signal of moderate effect size emerges. Pd at $n_{\text{pairs}}=1, \overline{\cos}=0.18$ and Ta/Nb at $n_{\text{pairs}}=1, \overline{\cos} \approx 0.99$ are noisy in opposite directions; once both are removed, the d-band correlation that the full sample averages to zero becomes visible. This is itself a Simpson's-paradox structure on our own analysis: a small confounder ($n_{\text{pairs}}$ outliers) drove a real underlying effect to apparent null.

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig6_dband_closure.pdf}
\caption{Cross-style alignment by competing covariate. \textbf{(a)} d-electron count vs.\ mean cross-style PC1 alignment: full-sample Spearman $\rho = -0.018$ ($p = 0.95$); on the controlled $n_{\text{pairs}} \geq 3$ subset (n=12) $\rho = +0.523$ ($p = 0.087$). \textbf{(b)} $n_{\text{pairs}}$ vs.\ mean alignment on log scale: full-sample $\rho = -0.497$ ($p = 0.060$); controlled subset $\rho = -0.663$ ($p = 0.023$). The single-pair outliers Pd (closed-shell, very low alignment) and Ta/Nb (open-shell, very high) drive the full-sample d-band correlation to zero by acting in opposite directions; removing them reveals the underlying physical signal.}
\label{fig:dband}
\end{figure*}

The practical conclusion: the per-element alignment dichotomy is real as a numerical phenomenon, but the dominant predictor is sample size, not d-band fullness. A residual d-band effect of moderate magnitude likely exists and warrants confirmation on a larger element set with controlled fitting depth ($n_{\text{pairs}} \geq 6$). The methodological lesson generalizes: any cross-element benchmark that does not control for fitting depth will conflate sampling effects with element-intrinsic physics. We recommend reporting $n_{\text{pairs}}$ or its analog alongside any per-element accuracy metric, and using weighted estimators that down-weight low-pair-count entries.

\subsection{Foundation MLIP extension and cross-paradigm universality}
\label{subsec:foundation_mlip}

The preceding sections establish that classical interatomic potentials exhibit structured, low-dimensional error manifolds (hyper-ribbons) and that cross-style alignment patterns are dominated by sample-size confounders rather than electronic-structure determinants. A critical question remains: are these geometric properties intrinsic to the classical potential form, or do they reflect deeper constraints of the materials themselves? To test the paradigm-independence of the hyper-ribbon structure, this study extends the analysis to three foundation machine-learning interatomic potentials (MLIPs)---pre-trained neural network potentials that are architecture-agnostic and trained on large-scale density functional theory (DFT) datasets. Specifically, MACE-MP-0 (small)~\cite{batzner2022}, a message-passing neural network trained on the MPtrj dataset; CHGNet~\cite{deng2023}, a crystal Hamiltonian graph neural network; and Orb-v3~\cite{kolluru2024}, an orbital-based equivariant neural network, were evaluated on the same 15 elemental metals.

Elastic constants ($C_{11}$, $C_{12}$, $C_{44}$) were computed via the strain-energy method with a maximum strain of $\varepsilon_{\max} = 0.5\%$, using the same reference data as the classical analysis (Simmons \& Wang~\cite{simmons1971} for FCC metals and Materials Project data supplemented by Simmons \& Wang for BCC metals). Relative-error vectors $\vec{\delta} = (\delta_{11}, \delta_{12}, \delta_{44})$ were constructed with $\delta_{ij} = (C_{ij}^{\text{pred}} / C_{ij}^{\text{ref}}) - 1$, normalized to unit length, and all pairwise cosine similarities between MLIP error vectors were computed per element.

The cross-MLIP alignment varies dramatically by element, as shown in Table~\ref{tab:foundation_classical_comparison}. Several elements exhibit near-perfect agreement among foundation models: Au ($0.984$), Pt ($0.934$), Ta ($0.969$), and Ag ($0.950$) all display high pairwise cosine similarities, indicating that distinct neural network architectures commit nearly identical directional errors in elastic constant space. Conversely, certain transition metals show negative or near-zero alignment: Cr ($-0.200$), V ($-0.249$), and Nb ($0.022$) exhibit MLIP error vectors that point in divergent or even opposing directions. Notably, Cr and V---both BCC refractory metals with complex magnetic and electronic structure---show the weakest cross-MLIP agreement, suggesting that these elements challenge the representational capacity of foundation models in ways that are architecture-dependent.

\begin{table}[htbp]
\centering
\caption{Per-element comparison of classical cross-style mean cosine similarity and foundation MLIP mean cosine similarity. Elements are ordered by decreasing classical mean cosine. The FCC/BCC crystal structure label is included for reference. Bold values highlight the higher of the two means per element.}
\label{tab:foundation_classical_comparison}
\begin{tabular}{lcccc}
\toprule
Element & Structure & Classical Mean & MLIP Mean & MLIP Range \\
\midrule
Ta & BCC & \textbf{0.990} & 0.969 & $[0.938,\,0.988]$ \\
Nb & BCC & \textbf{0.976} & 0.022 & $[-0.511,\,0.975]$ \\
Au & FCC & 0.948 & \textbf{0.984} & $[0.971,\,0.992]$ \\
Ag & FCC & 0.906 & \textbf{0.950} & $[0.922,\,0.999]$ \\
Cr & BCC & \textbf{0.904} & $-0.200$ & $[-0.787,\,0.938]$ \\
Pb & FCC & \textbf{0.885} & 0.560 & $[0.200,\,0.861]$ \\
Pt & FCC & 0.853 & \textbf{0.934} & $[0.888,\,0.990]$ \\
Cu & FCC & \textbf{0.796} & 0.671 & $[0.426,\,0.964]$ \\
Mo & BCC & \textbf{0.775} & 0.346 & $[-0.131,\,0.913]$ \\
V & BCC & \textbf{0.744} & $-0.249$ & $[-0.877,\,0.986]$ \\
Ni & FCC & \textbf{0.689} & 0.377 & $[0.189,\,0.578]$ \\
Fe & BCC & 0.618 & \textbf{0.747} & $[0.527,\,0.910]$ \\
W & BCC & 0.587 & \textbf{0.607} & $[0.167,\,0.946]$ \\
Al & FCC & 0.454 & \textbf{0.805} & $[0.653,\,0.975]$ \\
Pd & FCC & 0.184 & \textbf{0.748} & $[0.540,\,0.920]$ \\
\bottomrule
\end{tabular}
\end{table}

Crucially, the classical cross-style alignment pattern does not transfer to foundation MLIPs. The Spearman rank correlation between the classical mean cosine and the MLIP mean cosine across all 15 elements is $\rho = 0.186$ ($p = 0.508$), indicating no statistically significant relationship between the two paradigms. This decoupling is further illustrated by the group-level analysis: elements classified as ``strong alignment'' under the classical regime (Ta, Nb, Au, Ag, Cr, Pb, Pt) exhibit a mean MLIP cosine of $0.603$, while elements classified as ``weak alignment'' (Pd, Al, W, Fe) show a virtually identical mean of $0.607$. The classical grouping thus possesses zero predictive power for MLIP behavior. Paradigm-specific factors---the functional form of classical potentials versus the neural network architecture and training data of foundation MLIPs---overwhelm any element-specific determinism in the error geometry.

Iron persists as a notable outlier across both paradigms. Under classical potentials, Fe displays relatively weak cross-style alignment ($0.618$), and under foundation MLIPs it shows the highest internal variability among the BCC metals (cosine range $[0.527,\,0.910]$). This cross-paradigm consistency suggests that the source of Fe's anomalous behavior lies in the material itself---likely its complex magnetic ground state and the sensitivity of elastic properties to spin ordering---rather than in any particular modeling framework.

Despite the lack of correlation between paradigms at the element level, the hyper-ribbon structure itself proves to be paradigm-independent. As illustrated in Fig.~\ref{fig:dimensionality}, the MLIP error vectors for each element collectively span low-dimensional manifolds with effective participation ratios comparable to those of classical potentials ($\text{PR} = 1.05$--$1.86$ out of 3). Whether the model is a classical embedded atom method (EAM) potential or a foundation neural network, the prediction errors in elastic constant space are not isotropically distributed but instead concentrate along structured, low-dimensional directions. This universality implies that the hyper-ribbon geometry reflects fundamental constraints imposed by the target property space and the crystalline reference state, rather than artifacts of any particular functional form. The finding carries practical significance: foundation MLIPs, despite their superior absolute accuracy, remain subject to the same geometric error structure that governs classical potentials, and any ensemble or calibration strategy designed to exploit this structure may transfer across modeling paradigms.
...
The natural mechanistic extension of this finding---that closed-shell d-band fullness might predict tighter cross-style PC1 alignment within elements, just as crystal structure predicts inter-property correlation---fails on the full sample (\S\ref{sec:dband}; $\rho = -0.018$, $p = 0.95$). The dominant per-element predictor of cross-style alignment is instead the number of pair\_style families fitted to each element ($\rho(n_{\text{pairs}}, \overline{\cos}) = -0.50$, $p = 0.06$), and the apparent d-band signal recovers only on a controlled $n_{\text{pairs}} \geq 3$ subset ($\rho = +0.52$, $p = 0.087$). This is a Simpson's-paradox structure on our own analysis: outlier elements with $n_{\text{pairs}} = 1$ act in opposite directions and zero the full-sample correlation. The methodological implication for benchmarking is that fitting depth must be controlled---or down-weighted---before claiming any element-intrinsic property from cross-style PC analysis.
