...
\textbf{Results:} FCC errors occupy a hyper-ribbon manifold with effective dimensionality $1.17\pm0.24$ (LJ) to $1.38\pm0.50$ (SW) out of 3, consistent with the sloppy model universality class ($R^2>0.94$ for log-linear eigenvalue spacing). Pooled BCC data produce a spurious correlation inversion ($r=-0.435$ pooled vs. $+0.147$ within-group) attributable to element identity as a causal confounder. Meta-analysis reveals high heterogeneity ($I^2=91.4\%$, $\tau=0.48$), mandating random-effects models.
...
\end{itemize}
Reference values are taken from the single-crystal elastic constants handbook of Simmons and Wang\cite{simmons1971} and supplemented by data from the OpenKIM repository\cite{openkim}. Predictions were generated using three potentials: embedded-atom method (EAM), Lennard-Jones (LJ), and Stillinger-Weber (SW).
...
Table~\ref{tab:hyperribbon} summarizes the hyper-ribbon analysis for FCC elastic constant errors. All three potentials satisfy the hyper-ribbon criteria: strong monotonic decay ($\tau = -1.000$), high log-linearity ($R^2 > 0.94$), and fractional dimensionality $< 0.9$.
...
\midrule
EAM  & 8 & 1.37 & [1.14, 2.16] & 0.940 & $-1.000$ & Yes \\
LJ   & 8 & 1.17 & [1.04, 1.51] & 0.991 & $-1.000$ & Yes \\
SW   & 8 & 1.38 & [1.09, 2.08] & 0.985 & $-1.000$ & Yes \\
\bottomrule
\end{tabular}
\end{table}

The eigenvalue spectra (Fig.~\ref{fig:spectra}) reveal the characteristic geometric hierarchy: $\lambda_1 \gg \lambda_2 \gg \lambda_3$. The LJ potential shows the strongest compression (PR$=1.17$), with 92.3\% of error variance captured by the first principal direction. SW is least compressed (PR$=1.38$), consistent with its three-body functional form providing an additional degree of freedom.
...
Random-effects meta-analysis of per-material error correlations reveals substantial heterogeneity (Table~\ref{tab:meta}). The fixed-effects model yields a pooled correlation of $r = 0.54$ (95\% CI: $0.45$--$0.62$), while the random-effects model gives $r = 0.54$ (95\% CI: $0.20$--$0.76$) with a wide prediction interval ($-0.39$ to $+0.92$).
...
\midrule
Fixed  & $+0.54$ & $[0.45, 0.62]$ & 58.2 & 91.4\% & $0.00$ \\
Random & $+0.54$ & $[0.20, 0.76]$ & 58.2 & 91.4\% & $0.48$ \\
\bottomrule
\end{tabular}
\end{table}

The forest plot (Fig.~\ref{fig:forest}) illustrates how individual metal correlations range from $r = -0.15$ (Pt) to $r = +0.85$ (Al), and how the random-effects model appropriately widens the confidence interval from $[0.45, 0.62]$ to $[0.20, 0.76]$ to account for between-study variance.
...
\textbf{First, errors are structured, not noise.} The FCC elastic constant errors for all three potentials occupy hyper-ribbon manifolds with effective dimensionality $1.2$--$1.4$ out of $3$. This is the signature of sloppy model universality: many parameter combinations are unobservable, and only a few stiff directions control predictions\cite{transtrum2011}. The implication for potential design is that ``universal'' potentials cannot be universal in parameter space---they must be universal on the low-dimensional prediction manifold.
...
\textbf{Third, meta-analysis must use random-effects models.} The high $I^2$ values ($>91\%$) indicate that most variance is between-group rather than within-group. Fixed-effects models, which assume a single true effect, systematically underestimate uncertainty and can produce misleadingly narrow confidence intervals. The prediction intervals from random-effects models properly communicate the expected range for a new material.

\textbf{Limitations.} Our benchmark sets are small ($8$ FCC and $7$ BCC metals). The conclusions should be validated against larger corpora such as OpenKIM\cite{openkim} or the NIST Interatomic Potential Database. We have also restricted analysis to classical potentials; recent neural network potentials (NNIPs) may exhibit different manifold structure, though Mao et al.\cite{mao2024,mao2026} have reported similar low-dimensional error compression for deep networks. The applicability of Simpson's paradox detection to physical sciences remains rare\cite{selvitella2017}, making further validation in materials-specific contexts particularly valuable.
...
All benchmark datasets, analysis code, and figure generation scripts are available at \url{https://github.com/alexwelcing/lupine/tree/main/atlas-distill} under the MIT License. The \texttt{atlas-distill} engine is a standalone Rust binary. Precomputed analysis outputs (JSON format) are included in the repository. Reference elastic constant data are sourced from Simmons and Wang\cite{simmons1971} and the OpenKIM repository\cite{openkim}.
