...
export const Route = createFileRoute('/pilots')({
  ...
  head: () => ({
    meta: [
      { title: 'Pilots — three named wedges for the audit layer' },
      { name: 'description', content: 'Solid-state electrolytes, Ni-base superalloys, and electrocatalysts. Three wedges where the cross-potential audit pays off immediately, on the customer\'s own MLIP candidates and benchmark, without migration.' },
      { property: 'og:title', content: 'Pilots — three named wedges for the audit layer' },
      { property: 'og:description', content: 'Pick a wedge. We run the audit beside your MLIPs. No swap-out.' },
...
const WEDGES = [
  {
    ...
    deliverable:
      'Cross-potential audit across the customer-defined MLIP set (typically 6–10 candidates from UMA / MACE-MP / Orb / SevenNet plus internal fine-tunes), localized to migration-barrier curvature, written back as a per-trajectory error budget that travels with every NEB run.',
  ...
  },
  {
    ...
    deliverable:
      'Phase-stratified random-effects meta-analysis with Simpson\'s-paradox detection. We refuse to declare a global winner when the within-phase evidence does not support one — and we report the within-phase ranking the aggregate would have hidden.',
  ...
  },
  {
    ...
    deliverable:
      'OOD detection on adsorbate-coverage configurations, plus property-specific uncertainty for binding energies and reaction barriers. The customer keeps their MLIP; we attach a calibrated confidence to every screening prediction.',
...
function PilotsPage() {
  return (
    <PageShell
      kicker="PILOTS · NAMED WEDGES"
      title="Pick a wedge."
      subtitle="We do not pilot 'materials informatics in general.' We pilot one named workflow at a time — solid-state electrolytes, Ni-base superalloys, electrocatalysis — and we run the cross-potential audit against the MLIPs the customer already trusts. No migration, no swap-out, no synthesis claims."
      maxWidth="5xl"
    >
